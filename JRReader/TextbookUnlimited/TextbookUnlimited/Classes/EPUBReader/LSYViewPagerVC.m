...
@interface LSYViewPagerVC ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource>
{
    ...
    UIView *m_pHeaderBottomLine;//头部底部边框
    ...
    if ([self.dataSource respondsToSelector:@selector(numberOfViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<numberOfViewController; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexOfViewControllers:)]) {
                ...
                //zwq bgn 修改头部文字为背景图片
                //crj 修改背景图片为文字 20181128
//                NSArray* pAry = @[@"book_catalog2",@"book_note2",@"book_label2"];
                NSArray* pAry = @[@"目录",@"书签",@"笔记"];
                NSString*strBookTitle = pAry[i];
                //zwq end
             ...
             //   [button setTitle:buttonTitle forState:UIControlStateNormal];
                //crj调整侧滑栏UI的目录、笔记、标签为image
//                [button setImage:[UIImage imageNamed:strBookTitle] forState:UIControlStateNormal];
//                [button setImage:[UIImage imageNamed:[strBookTitle stringByReplacingOccurrencesOfString:@"2" withString:@"1"]] forState:UIControlStateSelected];
                //crj 调整侧滑栏UI的目录、笔记、标签为文字 20181128
                [button setTitle:strBookTitle forState:UIControlStateNormal];
                //crj end
...
-(void)viewDidLayoutSubviews
{
    ...
    _titleBackground.frame = CGRectMake(0,
                                        (headerView.frame.size.height)?headerView.frame.origin.y+headerView.frame.size.height:self.topLayoutGuide.length,
                                        self.view.frame.size.width,
                                        [self.dataSource respondsToSelector:@selector(heightForTitleOfViewPager:)]?[self.dataSource heightForTitleOfViewPager:self]:0);
    //zwqbgn 设置header底部边框
    CGFloat fBorderH = 1 ;
    CGSize szHeaderBg = _titleBackground.frame.size;
    CGRect rtBLine = CGRectMake(0, szHeaderBg.height, szHeaderBg.width, fBorderH);
    if(m_pHeaderBottomLine){
        m_pHeaderBottomLine.frame = rtBLine;
    }else{
        m_pHeaderBottomLine = [[UIView alloc]initWithFrame:rtBLine];
//        m_pHeaderBottomLine.backgroundColor = UIColor.grayColor;
        //crj修改目录标签笔记下划线颜色20181128
        m_pHeaderBottomLine.backgroundColor = [UIColor colorWithHexString:@"#cccccc"];
        [_titleBackground addSubview:m_pHeaderBottomLine];
        _titleBackground.clipsToBounds = false;
    }
    //zwq end
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self setTitleColor:[UIColor colorWithHexString:@"e34066"] forState:UIControlStateSelected];
        [self setBackgroundColor:[UIColor whiteColor]];
    }
    return self;
}
//crj注释掉：联动的下划线
//crj撤销注释，修改lineWidth = 1.5
